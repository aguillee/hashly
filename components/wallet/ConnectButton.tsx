  ...
  if (!isConnected || !user) {
    ...
    return (
      <button
        ...
        disabled={isConnecting}
        className={cn(
          "flex items-center justify-center gap-2 w-full px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150",
          "bg-brand/10 text-brand hover:bg-brand/20 active:scale-[0.98]",
          "disabled:opacity-50 disabled:cursor-not-allowed"
        )}
      >
        <Wallet className="h-4 w-4 flex-shrink-0" weight="duotone" />
        {isConnecting ? (
